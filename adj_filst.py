...
def adjfilst(cps):
    ...
    cps['z1'] = cps.apply(lambda row: np.random.uniform(0, 1)
                          if row['case1'] == 1 else 1, axis=1)
    np.random.seed(5410)
    cps['z2'] = cps.apply(lambda row: np.random.uniform(0, 1)
                          if row['case2'] == 1 else 1, axis=1)
    selected = cps['z1'] <= 0.84 | cps['z2'] <= 0.54
    cps['filst'] = np.where(selected, 1, cps['filst'])
    cps.drop(['case1', 'case2'], axis=1, inplace=True)
    cps['cpsseq'] = cps.index + 1

    # Split the CPS into filers and nonfilers
    filers = copy.deepcopy(cps[cps['filst'] == 1])
    nonfilers = copy.deepcopy(cps[cps['filst'] == 0])
