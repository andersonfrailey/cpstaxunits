...
def p_recs(rec):
    ...
    record['m5g_cbst'] = [int(rec[1073])]
    record['m5g_dscp'] = [int(rec[1074])]
    record['m5gsame'] = [int(rec[1075])]
    record['m5g_reg'] = [int(rec[1076])]
    record['m5g_st'] = [int(rec[1077:1079])]
    record['m5g_div'] = [int(rec[1079:1081])]
    record['m5g_mtr1'] = [int(rec[1081:1083])]
    record['m5g_mtr3'] = [int(rec[1083])]
    record['m5g_mtr4'] = [int(rec[1084])]
    record['i_m5g1'] = [int(rec[1085])]
    record['i_m5g2'] = [int(rec[1086:1088])]
    record['i_m5g3'] = [int(rec[1088])]

    return pd.DataFrame(record)


def create_cps(raw_cps, export=False):
    """
    ...
    # Create data set by combining all of the records
    cps_mar = pd.concat(cps_list)
    # Export the data if specified
    if export:
        print('Exporting Data')
        cps_mar.to_csv('data/cpsmar2015.csv', index=False)
    return cps_mar
